...
//$atts['href']        = '#';
			...
			$item_output .= $args->link_before . apply_filters( 'the_title', $item->title,
				...
				/*
					$item_output .= ( $args->has_children && 0 === $depth ) ? 	'<button type="button" class="btn dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span>
					</button></a>' :  '</a>';*/
					$item_output .= ( $args->has_children && 0 === $depth ) ? 	'</a><button type="button"  class="btn dropdown-toggle" aria-haspopup="true" aria-expanded="false" ><span class="sr-only">'.__('הצג תפריט משנה עבור'). $item->title .'</span>
					</button>' :  '</a>';
